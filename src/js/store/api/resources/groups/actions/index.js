...
export function enableBatch(actions) {
  return names => dispatch => {
    dispatch(actions.groups.batch_action('setStatus', names.join(','), 'enabled=true', { enabled: true }, 'groups'));
  };
}

export function disableBatch(actions) {
  return names => dispatch => {
    dispatch(actions.groups.batch_action('setStatus', names.join(','), 'enabled=false', { enabled: false }, 'groups'));
  };
}
