...
function setOptionsPayload(workflow, name, value) {
  ...
  return fetchJson('PUT', `${settings.REST_BASE_URL}/workflows/${workflow.id}`, {
    body: JSON.stringify({
      action: 'setOptions',
      options: `${name}=${value}`,
    }),
...
const setOptions = createAction('WORKFLOWS_SETOPTIONS', setOptionsPayload, setOptionsMeta);
...
function fetchLibSourcesPayload(workflow, date) {
  ...
  return fetchJson('GET', `${settings.REST_BASE_URL}/workflows/${id}?lib_source=true&date=${date}`);
}
...
const addNew = createAction('WORKFLOWS_ADDNEW', async (id) => {
  // @ts-ignore ts-migrate(2554) FIXME: Expected 5 arguments, but got 2.
  const wf = await fetchJson('GET', `${settings.REST_BASE_URL}/workflows/${id}`);
...
const processOrderEvent = createAction('WORKFLOWS_PROCESSORDEREVENT', (events) => ({ events }));
...
const processMemoryChanged = createAction('WORKFLOWS_PROCESSMEMORYCHANGED', (events) => ({
  events,
}));
...
const toggleEnabled = createAction('WORKFLOWS_ENABLE', (ids, value, dispatch) => {
...
const fetchList = createAction('WORKFLOWS_FETCHLIST', async () => {
  // @ts-ignore ts-migrate(2554) FIXME: Expected 5 arguments, but got 2.
  const result = await fetchJson('GET', `${settings.REST_BASE_URL}/workflows?list=1`);
...
const reset = createAction('WORKFLOWS_RESETCALL', async (ids, dispatch) => {
  ...
  fetchWithNotifications(
    async () =>
      // @ts-ignore ts-migrate(2554) FIXME: Expected 5 arguments, but got 2.
      await fetchJson('PUT', `${settings.REST_BASE_URL}/workflows?ids=${id}&action=reset`),
...
const toggleStart = createAction('WORKFLOWS_STARTCALL', (ids, type, dispatch) => {
  const id = isArray(ids) ? ids.join(',') : ids;

  fetchWithNotifications(
    async () =>
      // @ts-ignore ts-migrate(2554) FIXME: Expected 5 arguments, but got 2.
      await fetchJson('PUT', `${settings.REST_BASE_URL}/workflows?ids=${id}&action=${type}`),
...
const toggleDeprecated = createAction('WORKFLOWS_TOGGLEDEPRECATED', (ids, value, dispatch) => {
  const id = isArray(ids) ? ids.join(',') : ids;
...
const setAutostart = createAction('WORKFLOWS_SETAUTOSTART', (id, value, dispatch) => {
  fetchWithNotifications(
    async () =>
      // @ts-ignore ts-migrate(2554) FIXME: Expected 5 arguments, but got 2.
      await fetchJson(
    ...
    `${value ? 'Setting' : 'Unsetting'} workflow ${id} autostart to ${value}...`,
    `Workflow ${id} autostart set to ${value}`,
    dispatch
  );
});

const setThreshold = createAction('WORKFLOWS_SETTHRESHOLD', (id, value, dispatch) => {
  fetchWithNotifications(
    async () =>
      // @ts-ignore ts-migrate(2554) FIXME: Expected 5 arguments, but got 2.
      await fetchJson(
...
const setRemote = createAction('WORKFLOWS_SETREMOTE', (id, value, dispatch) => {
  ...
  fetchWithNotifications(
    async () =>
      ...
      await fetchJson(
        'PUT',
        `${settings.REST_BASE_URL}/workflows/${id}?action=setRemote&remote=${value ? 1 : 0}`
      ),
